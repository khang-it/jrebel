...
import com.vvvtimes.util.IpUtil;
...
public class MainServer extends AbstractHandler {
    ...
    private static Map<String,String> licenseSrverInfoMap = new HashMap<String, String>();
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("获取到最终的服务器IP地址："+ip);
        String str = String.format("http://%s:%s", ip, port);
        String str1 = String.format("License Server started at %s", str);
        String str2 = String.format("JetBrains Activation address was: %s/",str);
        String str3 = String.format("JRebel 7.1 and earlier version Activation address was: %s/{tokenname}, with any email.",str);
        String guid = UUID.randomUUID().toString();
        String str4 = String.format("JRebel 2018.1 and later version Activation address was: %s/{guid}(eg:%s/%s), with any email.",str,str,guid);
        licenseSrverInfoMap.put("str1",str1);
        licenseSrverInfoMap.put("str2",str2);
        licenseSrverInfoMap.put("str3",str3);
        licenseSrverInfoMap.put("str4",str4);
    ...
    private void indexHandler(String target, Request baseRequest, HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        response.getWriter().println(String.format("<h3>%s</h3>",licenseSrverInfoMap.get("str1")));
        response.getWriter().println(String.format("<h3>%s</h3>",licenseSrverInfoMap.get("str2")));
        response.getWriter().println(String.format("<h3>%s</h3>",licenseSrverInfoMap.get("str3")));
        response.getWriter().println(String.format("<h3>%s</h3>",licenseSrverInfoMap.get("str4")));
    }
}